...
def parser(subparsers):
    parser = subparsers.add_parser(
        "upkgbin", aliases=["u"], help="multi bin package to upgrade"
    )
    ...
    parser.add_argument(
        "-b", "--bin", type=str, action="append", default=[], help="bin file list"
    )
...
class BinFile:
    def __init__(self, path, addr, flag):
        ...
        self.header = (
            struct.pack(">H", self.flag)
            + struct.pack(">II", self.addr, self.size)
            + self.hash
        )
        self.check = _crc16(self.header)
...
def action(args):
    ...
    upgrade_header = (
        struct.pack(">IH", args.magic, bin_count)
        + struct.pack(">I", total_size)
        + upgrade_hash.digest()
    )
    upgrade_check = _crc16(upgrade_header)
    upgrade_header = struct.pack(">H", int(upgrade_check)) + upgrade_header

    upkg_output = args.output
    debug(f"[upkgbin] count:{bin_count} into:{upkg_output}")
    ...
    with open(upkg_output, "wb") as f:
        ...
        for i, b in enumerate(bin_list):
            ...
            debug(
                f"""- bin {i+1}: {b.path}, addr:{hex(b.addr)}, flag:{
    ...
    if args.limit > 0 and upkg_filesz > args.limit:
        ...
        debug(f"[upkgbin] upkg:{upkg_filesz} limit:{args.limit} rate:{limit_rate:.2f}%")
...
def _type_val(x):
    try:
        ...
        error(f"`{x}` not a uint16 hex integer")
...
def _type_hex(x):
    try:
        ...
        error(f"`{x}` not a invalid hex integer")
